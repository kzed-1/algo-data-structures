...
function reverseLinkedList(linkedList) {
  ...
  // const originalList = [linkedList.head, null]

  // while (originalList[0].next !== null) {
  //   originalList.unshift(originalList[0].next);
  // }

  // for (let i = 0; i < originalList.length - 1; i++){
  //   originalList[i].next = originalList[i+1] 
  // }

  // linkedList.head = originalList[0]
  // linkedList.tail = originalList[originalList.length - 2]
  ...
  while (next = node.next) { // same as let next = node.next, while (next)
    node.next = prev;
    prev = node;
    node = next;
  }
